...
export default function SellerProductForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const productData = {
      title: formData.name,               // form calls it "name", backend expects "title"
      description: formData.description,
      price: parseFloat(formData.price),
      categoryId: null,                   // form has no category picker wired to IDs yet
      imageUrls: validImages,
      variants: [{                        // backend requires at least 1 variant
        size: null,
        color: null,
  ...
  setErrors({ submit: error.response?.data?.message || 'Failed to save product. Try again.' });
} finally {
  setSaving(false);
}
  };
